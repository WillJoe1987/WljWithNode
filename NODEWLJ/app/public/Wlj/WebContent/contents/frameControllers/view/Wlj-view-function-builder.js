...
Ext.extend(Wlj.view.functions.app.Builder,Ext.util.Observable,{
	...
	getCodeFilePage : function(){
		if(parent.CUSTVIEW && parent.CUSTVIEW.CURRENT_VIEW_URL){
			this.codeFile = parent.CUSTVIEW.CURRENT_VIEW_URL;
		} else {
			var serviceMgr = parent.Wlj?parent.Wlj.ServiceMgr:undefined;
			var p = parent;
			for(var i=0;i<10 && !serviceMgr;i++){
				p = p.parent;
				serviceMgr = p.Wlj?p.Wlj.ServiceMgr:undefined;
			}
			if(serviceMgr.services.get('service_' + this.resId)){
				this.codeFile = serviceMgr.services.get('service_' + this.resId).menuData.ACTION;
			}else if(parent.telsalefitprodinfourl){
				this.codeFile = parent.telsalefitprodinfourl;
			}
		}
	},
