...
Wlj.search.App = function(){
	...
	_this.windowAnimate = APPUTIL.windowAnimate;
	_this.usingTips = APPUTIL.usingTips;
...
Ext.extend(Wlj.search.App, Ext.util.Observable,{});
...
Wlj.search.App.prototype.bootTaskBar = function(){
	this.taskBar = new Wlj.widgets.search.window.TaskBar({
		...
		windowAnimate : this.windowAnimate,
		appObject : this
	});
...
Wlj.search.App.prototype.bootTips = function(){
	var _this = this;
	_this.searchTips = {};
	_this.searchTips.HEADERTIP = new Wlj.widgets.search.tip.HeaderTip({
		appObject : _this
	});
};

Wlj.search.App.prototype.bootEvents = function(){
	this.addEvents({
		tilechanged : true,
		tilesaved : true
	});
	this.addListener('tilechanged', function(){
		this.addTips('HEADERTIP','CHANGING');
	});
	var _this = this;
	this.addListener('tilesaved', function(){
		this.removeTips('HEADERTIP');
	});
};
...
Wlj.search.App.prototype.saveTiles = function(){
	var _this = this;
	var results = [];
	...
	Ext.Ajax.request({
		...
		success : function(){
			_this.fireEvent('tilesaved',_this);
...
Wlj.search.App.prototype.addTips = function(type, messageObject){
	var _this = this;
	if(!_this.usingTips) return;
	if(!_this.searchTips){
		_this.bootTips();
	}
	if(_this.searchTips[type]){
		var tipO = _this.searchTips[type];
		tipO.addMessage(messageObject);
	}
};
Wlj.search.App.prototype.removeTips = function(type, messageObject){
	var _this = this;
	if(!_this.usingTips) return;
	if(!_this.searchTips) 
		return;
	if(!_this.searchTips[type]) 
		return;
	var tipO = _this.searchTips[type];
	tipO.removeMessage(messageObject);
};